...
void ZMST::dfs(int root, int father, int stat, layout& lay,
  ...
  if (stat == head[root + 1]) {
    ...
    for (unsigned i = 0; (stack[id] = i) < subProb[son].size(); i ++) {
...
void ZMST::exec() {
  ...
  for (unsigned i = 0; i < smst.lines().size(); i ++)
  ...
  for (unsigned i = 0, j = 0; i <= points().size() && j <= lines().size(); ) {
  ...
  for (vector<Line_Z>::iterator it = lines_.begin();
    ...
    int minx = min(point(it->start()).x(), point(it->end()).x()),
        ...
        maxy = max(point(it->start()).y(), point(it->end()).x());
